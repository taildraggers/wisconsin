...
Daily aggregator of Wisconsin-located taildragger classified listings,
...
[West Virginia](https://github.com/taildraggers/west-virginia) repos'
approach, with the state filter swapped to "WI".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, WI` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Wisconsin"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/wisconsin/`
...
```html
<iframe
  src="https://taildraggers.github.io/wisconsin/"
  title="Taildragger Ads in Wisconsin"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_WISCONSIN_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
