"""Aggregates Wisconsin-located listings from every taildraggers.com
...
ones whose location ends in ", WI".
...
Vermont/Virginia/Washington/West Virginia repos' approach, with the state
filter swapped to "WI".
"""
...
PAGE_TITLE = "Taildragger Ads in Wisconsin"
STATE_ABBR = "WI"
...
_WISCONSIN_LOCATION_RE = re.compile(r",\s*WI\s*$", re.IGNORECASE)


def _is_wisconsin(location: str) -> bool:
    return bool(_WISCONSIN_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        wisconsin_rows = [row for row in rows if _is_wisconsin(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(wisconsin_rows)} in Wisconsin")
        all_listings.extend(wisconsin_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Wisconsin listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Wisconsin listings: {len(listings)}")
